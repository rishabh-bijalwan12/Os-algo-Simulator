...
const Fifo = () => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      alert('Failed to calculate FIFO. Please check your inputs.');
    }
  };
  ...
  const chartData = result ? {
    labels: result.steps.map((step, idx) => `Step ${idx + 1}`),
    datasets: [
      {
        label: 'Page Faults',
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="space-y-6">
          <div className="bg-white p-6 rounded-lg shadow-md">
            <h1 className="text-2xl font-bold text-gray-800 mb-6">FIFO Page Replacement</h1>
        ...
        {/* Right Panel: Charts */}
        <div className="space-y-6">
          {chartData && (<BarChart data={chartData} title="FIFO Page Faults" />)}
          {compareData && <CompareMemoryChart data={compareData} />}
          {compareData && <PieChart data={compareData} />}  {/* Add Pie Chart here */}
        </div>
      </div>
    </div>
  );
