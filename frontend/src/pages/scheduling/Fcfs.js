...
const Fcfs = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      alert('Failed to calculate FCFS.');
    }
  };
                    ...
                    <table className="w-full text-sm text-left">
                      <thead className="bg-gray-200 text-gray-700">
                        <tr>
                          ...
                          <th className="px-3 py-2">Avg Waiting Time</th>
                          <th className="px-3 py-2">Avg Turnaround Time</th>
                        </tr>
                      </thead>
                      <tbody>
                        {Object.entries(compareData).map(([algo, data]) => (
                          <tr key={algo} className="border-t">
                            ...
                            <td className="px-3 py-2">{data.avgWaitingTime}</td>
                            <td className="px-3 py-2">{data.avgTurnaroundTime}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
