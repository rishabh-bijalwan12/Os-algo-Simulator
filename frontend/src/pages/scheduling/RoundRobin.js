...
const RoundRobin = () => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      alert('Failed to calculate Round Robin.');
    }
  };
    ...
    <div className="min-h-screen bg-gray-100 p-6">
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white p-6 rounded-lg shadow-md">
          <h1 className="text-2xl font-bold text-gray-800 mb-4">Round Robin Scheduling</h1>

          <div className="mb-4">
            <label className="block text-sm font-medium text-gray-700 mb-1">Quantum Time</label>
            <input
              type="number"
              min="1"
              value={quantum}
          ...
          {result && (
            <div className="mt-6">
              ...
              <div className="mt-4">
                ...
                <div className="flex flex-nowrap items-center bg-gray-100 rounded overflow-x-auto p-2">
                  {result.ganttChart.map((item, idx) => (
                    <div
                      key={idx}
                      className="flex flex-col justify-center items-center border-r border-white text-xs px-2 py-1"
                      style={{
                        minWidth: `${(item.endTime - item.startTime) * 30}px`,
